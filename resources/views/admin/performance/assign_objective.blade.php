    ...
    $(document).ready(function() {
        var datatable = $table.dataTable({
                ...
                "ajax": {
                    "type": "POST",
                    "async": true,
                    "url": '{{url("/admin/assign_objective")}}',
                    "data": {},
                    "dataSrc": "data",
                    "dataType": "json",
                    "cache": false,
                ...
                "columnDefs": [
                    ...
                    "createdCell": function (td, cellData, rowData, row, col) {
                        html = '';
                        if(rowData.manager_id==null){
                            html="Not Assigned";
                        }else {
                            if (rowData.manager_id==0 && rowData.employee_id==0) {
                                html= "{{Session::get('user')['full_name']}}"+"(Admin)";
                    ...
                    orderable: false,
                    "createdCell": function (td, cellData, rowData, row, col) {
                        html = '<a href="{{url('admin/edit_objective/')}}/'+cellData+'"><input class="btn btn-info" type="button" value="Assign"></a>'
                        $(td).html(html);
                    }
                }],
...
var e='{{Session::get('assign_error_msg')}}';
          if(e !=''){
            Swal.fire(e);
          }
</script>